...
class EMTMDEHProblem(PhysicsProblem):
    """
    EMTMD_EH inverse design problem.
    Find the optimal values for the inductances (L) and resistances (R) of an electromechanical structure composed of
    9 electromechanical TMDs attached in a clamped-free rod, to maximize the H2 norm of the energy harvesting efficiency
    (electrical power generated / mechanical power input)
    """
    ...
    def __init__(self):
        ...
        self.bounds = [(1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3), (1e-3, 0.3),
    ...
    def load_data(self, path):
        inp = torch.zeros((1,1))
        obs = 50*torch.ones((1,1))     # Maximum efficiency is set to 50 %
        return inp, obs
